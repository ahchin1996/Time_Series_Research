...
# load dataset
dataset = pd.read_csv('D:/Data/stock/DJI/^DJI_2011.csv', header=0, index_col=0)
df = pd.read_csv('D:/Data/stock/DJI/^DJI_2011.csv',header = None,sep= ",")
df.columns = ["Date",'Adj Close',"Open",'High','Low','Close']
x = df['Date']
x = x.astype('string')
...
for i in range(rumtime): 
    train, test = train_test_split(values, random_state=0, train_size=0.8)
    print(train)
    print(test)
    ...
    model.add(layers.Dense(64, input_shape=(train_X.shape[1],train_X.shape[2]), activation="sigmoid"))
    ...
    pyplot.plot(test_y, color =  'red', label = 'Real  Stock Price')  # 紅線表示真實登革熱數
    pyplot.plot(testPredict, color = 'blue', label = 'Predicted Stock Proce')  # 藍線表示預測登革熱數
    pyplot.xlabel('Time')
    pyplot.xticks
    pyplot.ylabel('Stock Index')
    pyplot.legend()
    pyplot.show()

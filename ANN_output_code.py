...
def get_result(path,fd,fd_2):
    ...
    model = Sequential()
    model.add(layers.Dense(64, input_shape=(training_data.shape[1],), activation="sigmoid"))
    ...
    # fit network
    history = model.fit(training_data, training_label, epochs=500, batch_size=128, validation_data=(test_data, test_label), verbose=2,shuffle=False)

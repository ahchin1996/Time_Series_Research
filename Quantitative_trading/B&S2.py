...
def Profitability(forecast_path,origin_path):
    ...
    for i in range(0,len(origin_close)-1):
        #buy
        if (forecast_close[i + 1] - origin_close[i]) <  origin_close[i] * .01   and stock == 0:
            stock += 1
            sig.append(1)
        #sell
        elif (forecast_close[i + 1] - origin_close[i]) >  origin_close[i] * .01   and stock == 1:
            stock -= 1
            sig.append(-1)
        else:
            sig.append(0)
    ...
    for i in range(len(sig)):
        ...
        if sig[i] == 1 :
            # 隔日收盤買入
            buy_price = origin_close[sig.index[i+1]]
            stock += 1
            # 紀錄交易日期
            transaction.append([sig.index[i+1],'buy'])
        elif sig[i] == -1 and stock == 1:
            # 隔日收盤賣出
            sell_price = origin_close[sig.index[i+1]]
            stock -= 1
    ...
    if stock == 1 and buy_price != 0 and sell_price == 0:
        sell_price = origin_close[len(sig)-1]
        rets.append((sell_price-buy_price)/buy_price)
        stock -= 1
        profit = profit + (sell_price - buy_price)

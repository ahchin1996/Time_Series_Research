...
def get_result(path,fd,fd_2):
    # path = 'D:/chin/new_data_2/DJI/DJI_2000.csv'
    ...
    split_no = 0
    while date_array.iloc[split_no] < datetime(year, 10, 1, 0, 0):
        split_no +=1
    print(split_no)
...
path = 'D:/chin/new_data_2'
output = find_fd(path)
finish_time = time.time()
print('Total times : {:.3f}'.format(finish_time-start_time))

output.to_csv(os.path.join(path, 'GSPC_OneAhead_Step.csv'), index=0, header=1)

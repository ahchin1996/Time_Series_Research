...
# load dataset
dataset = read_csv('D:/Data/Stock/new_data/^TWII/^TWII_2000.csv', header=0 , index_col=0)
...
rumtime=10
score=[[0]*5 for i in range(rumtime)]
...
for i in range(rumtime): 
    ...
    pyplot.plot(test_y,color ='red', label = 'Real Stock')  # 紅線表示真實登革熱數
    pyplot.plot(testPredict, color = 'blue', label = 'Predicted Stock')  # 藍線表示預測登革熱數
    pyplot.xlabel('Time')
    pyplot.ylabel('Stock Points')
    pyplot.legend()
    pyplot.show()
    ...
    score[i][1]=testScore
    score[i][2]=mape
    score[i][3]=trainPredict.tolist()
    score[i][4]=testPredict.tolist()
